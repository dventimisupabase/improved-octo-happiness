...
Because `transmute` moves no rows and creates no real partitions (`attain` does that, later), the
...
It is a one-way door the moment a real partition exists. As soon as `attain` (or `maintain`) has run,
