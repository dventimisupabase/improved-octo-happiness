...
-- ---- fixture: a table with many closed STRAY intervals for the assistant drain to work -------------
-- transmute moves the recent rows into the monolith and leaves the DEFAULT empty, so to exercise the
-- adaptive controller we seed the DEFAULT with backdated strays across many past days: each is a closed
-- daily interval the assistant drain evacuates, one per maintenance tick, giving the controller plenty
-- of ticks to ride the budget.
create table public.evt (created_at timestamptz not null, id bigint generated by default as identity,
                         body text, primary key (created_at, id));
insert into public.evt (created_at) select now() - (g || ' minutes')::interval from generate_series(1, 20) g;
select pgpm.transmute('public.evt', 'created_at', interval '1 day', p_obtain => 2, p_drain_batch => 8000, p_paused => false);
insert into public.evt (created_at, body)
  select date_trunc('day', now()) - make_interval(days => d), 'x'
  from generate_series(2, 12) d, generate_series(1, 80) g;
